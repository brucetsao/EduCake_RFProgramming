...
void loop() {
 connectstr = "" ;		//清空傳參數字串變數
   ...
   connectstr = "humidity=" + String((int)DHT.humidity)+"&"+"temperature=" + String((int)DHT.temperature);
     ...
     Serial.print("/") ;
     Serial.println(connectstr) ;		//印出傳參數字串變數資料
     
  	if (client.connect(server,80)) { // REPLACE WITH YOUR SERVER ADDRESS  連線到WEB伺服器，如果成功連接上
	       client.println("POST /iot/dhtdata/dhtadd.php HTTP/1.1"); 	//送出要求傳到/iot/dhtdata/dhtadd.php 程式的要求
                Serial.println("POST /iot/dhtdata/dhtadd.php HTTP/1.1"); 	//印出要求傳到/iot/dhtdata/dhtadd.php 的訊息

		client.print("Host:"); // SERVER ADDRESS HERE TOO		//送出要求的html
		 Serial.print("Host:"); // SERVER ADDRESS HERE TOO

		client.println(server); // SERVER ADDRESS HERE TOO		//送出要求的WEB伺服器網址
		 Serial.println(server); // SERVER ADDRESS HERE TOO

		client.println("Content-Type: application/x-www-form-urlencoded"); 		//送出要求的html
	  	 Serial.println("Content-Type: application/x-www-form-urlencoded"); 

		client.print("Content-Length: "); 		//送出要求的html
		 Serial.print("Content-Length: "); 

		client.println(connectstr.length()); 		//送出要求的html
		 Serial.println(connectstr.length()); 

		client.println(); 		//送出要求的html
		client.print(connectstr); 
 
		Serial.print(connectstr); 	//印出連接字串內容
                Serial.print("\n\n\n");		//印出三個換行鍵
                 	delay(1000); //延遲1000毫秒
	}
        else
